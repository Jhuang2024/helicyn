...
## Phase 5.1 addendum (premium motion pass)

Phase 5.1 added visual/motion polish to the five Phase 5 pages
(`partners.html`, `onboarding.html`, `login.html`, `partner-portal.html`,
`research.html`) and touched no auth/database/onboarding-submission logic.
New copy introduced by this pass:

| New copy | File / location | Notes |
|---|---|---|
...
| Rewrote `research.html`'s Status/Abstract/Methodology/Selected-findings sections from raw bracket-tag placeholders (`[PAPER_STATUS]`, `[ABSTRACT]`, `[METHODOLOGY_SUMMARY]`, `[KEY_FINDINGS]`, `[SELECTED_FIGURES]`, plus an internal authoring note "Replace this paragraph with...") into a proper "coming soon" panel (new `.comingsoon` component) | `research.html` | Same underlying facts as the pre-existing text (nothing is published yet; the working thesis/methodology summary is preliminary; findings and figures don't exist yet), just presented as an intentional UI state instead of unfilled template tags. Explicitly kept "simulated", "under assumptions", "research prototype", and the negation of production savings / real facility validation |

Re-ran the same `grep -rniE` sweep (plus `deployed`) across the five edited
pages and their new JS/CSS after the motion pass; no new matches outside
the pre-existing, already-audited occurrences above.
