...
(function () {
  ...
  function renderRecs(name) {
    // Recommendation cards are now owned and regenerated by control.js
    // (approve → simulate → file → regenerate). Scenario changes no longer
    // rewrite their text by index, which would fight that lifecycle.
    return;
  }
  ...
  function renderTopology(scn) {
    ...
    scn.flows.forEach((fl, i) => {
      ...
      base.setAttribute('data-from', fl.from); base.setAttribute('data-to', fl.to);
      ...
      dash.setAttribute('data-from', fl.from); dash.setAttribute('data-to', fl.to);
    ...
    // let the ambient layer re-wire node→flow illumination
    window.dispatchEvent(new CustomEvent('cp:topology-rendered'));
  }
  ...
  /* advance the coordination action counter (ACTION #NNN) on each new action */
  function bumpAction() {
    const el = $('[data-trace="action"]');
    if (!el) return;
    const m = el.textContent.match(/(\d+)/);
    const n = (m ? parseInt(m[1], 10) : 184) + 1;
    el.textContent = 'ACTION #' + n;
  }
  ...
  /* ---- apply an approved recommendation / workload action to the
  ...
  function applyRecPatch(recId) {
    ...
    tipUsed = {}; renderRegionCards(activeScene);
    if (rcards) { rcards.classList.remove('cp-flash'); void rcards.offsetWidth; rcards.classList.add('cp-flash'); }
    bumpAction();
  ...
  function runOptimization() {
    ...
    runBtn.querySelector('.t').textContent = 'Simulating…';
    ...
    timeline([
      ...
      { at: 0, fn: () => {
        ...
        showAlert('crit', 'Detect', 'Detected rising thermal and power constraints across selected regions.');
        pushEvent({ time: t(), type: 'detected', text: 'Detected rising thermal and power constraints across selected regions' });
      }},
      // 2 - ANALYZE
      { at: 1600, fn: () => {
        setStep('detect', 'done'); setStep('analyze', 'active');
        showAlert('info', 'Analyze', 'Evaluating workload flexibility, SLA protection, cooling headroom, carbon intensity, and regional capacity.');
        pushEvent({ time: t(), type: 'analyzed', text: 'Evaluating flexibility, SLA protection, cooling headroom, carbon, and capacity' });
      }},
      // 3 - ACT
      { at: 3400, fn: () => {
        setStep('analyze', 'done'); setStep('act', 'active');
        showAlert('info', 'Act', 'Generated operator-approved actions for workload shifting, deferral, rerouting, and thermal control.');
      ...
      { at: 5400, fn: () => {
        ...
        showAlert('ok', 'Verify', 'Estimated projected impact against the baseline simulation.');
      ...
      { at: 7000, fn: () => {
        ...
        bumpAction();
        CP.applyBump({ energy: 14, cost: 4300, carbon: 6, cooling: 1.1, gpu: 1, pue: -0.01 });
        bumpLifetime({ energy: 0.02, cost: 0.05, carbon: 6, gpuh: 0.01 });
        if (CP.revealVerification) CP.revealVerification();
        showAlert('ok', 'Save', 'Simulation complete. Recommendations added to the operator review queue.');
        pushEvent({ time: t(), type: 'saved', text: 'Recommendations added to operator review queue' });
      }},
      // finish
      { at: 8000, fn: () => {
        setStep('save', 'done');
        showAlert('ok', 'Simulation complete', 'Constraint resolved in simulation. Coordination holding across 5 regions.');
        runBtn.disabled = false;
        setSelectDisabled(false);
        runBtn.querySelector('.t').textContent = 'Run coordination simulation';
        running = false;
        setTimeout(() => {
  ...
  /* ============================================================
     ...
     ============================================================ */
  const life = { energy: 52.4, cost: 14.9, carbon: 18500, gpuh: 2.46 };
  function renderLifetime() {
    const set = (k, txt) => { const el = $('[data-life="' + k + '"]'); if (el) el.textContent = txt; };
    set('energy', CP.fmt(life.energy, 1));
    set('cost', CP.fmt(life.cost, 1));
    set('carbon', Math.round(life.carbon).toLocaleString('en-US'));
    set('gpuh', CP.fmt(life.gpuh, 2));
  }
  function bumpLifetime(d) { for (const k in d) life[k] += d[k]; renderLifetime(); }
  renderLifetime();
  if (!prm) {
    setInterval(() => {
      // gentle organic drift (fleet-wide)
      life.energy += 0.004 + Math.random() * 0.006;
      life.cost   += 0.003 + Math.random() * 0.005;
      life.carbon += 1.2   + Math.random() * 1.8;
      life.gpuh   += 0.0006 + Math.random() * 0.0010;
      renderLifetime();
    }, 2500);
  }
